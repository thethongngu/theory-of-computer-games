...
public:
    ...
    void add_piece(int pos, int color) {
        ...
        // position become 0-Go
        zero_pos[color].on_bit(pos);
        zero_pos[xxxxx].on_bit(pos);

        // position becomes 1 w.r.t color
        state[color].on_bit(pos);

        for (int nei: adj_cells[pos]) {  // get all neighbor pos
            if (is_occupied(nei, color)) {   // if neighbor is occupied by player

                // root of region contains neighbor piece
                int root_nei = get_root(nei);

                // combine liberties of 'pos' and nearby 'pos'
                lib_after |= lib[root_nei];

                // reset liberty of nearby 'pos'
                lib[root_nei].clear();
                lib_count[root_nei] = 0;

                // merge 2 region into 1
                merge(pos, root_nei);

            } else if (is_occupied(nei, xxxxx)) {  // if neighbor is occupied by enemy

                // root of region contains neighbor piece
                int root_nei = get_root(nei);

                // remove 1 liberty from this region (because we play at 'pos')
                lib[root_nei].off_bit(pos);
                lib_count[root_nei] = lib[root_nei].count();

                // if enemy region has only 1 liberty left
                if (lib_count[root_nei] == 1) {
                    zero_pos[color] |= lib[root_nei];  // player cannot move at position of bit 1
                    one_pos[xxxxx] |= lib[root_nei];   // only enemy can move at position of bit 1
                }

            } else {  // 'pos' is not occupied
                lib_after.on_bit(nei);      // player get one more liberty at 'nei'
                one_pos[xxxxx].on_bit(nei); // enemy can move at 'nei'
            }
        }

        lib_after.off_bit(pos);  // no liberty at 'pos'

        // calculate new liberties count of 'pos' region
        int root_pos = get_root(pos);
        lib[root_pos] = lib_after;
        lib_count[root_pos] = lib_after.count();

        // if player region has only 1 liberty left
    ...
    bool can_move(int pos, int color) {
        ...
        if (zero_pos[color].get(pos)) return false;  // capture or suicide move
        if (one_pos[color].get(pos) == 0) return true;  // quick check (not go through suicide check)

        one_pos[color].off_bit(pos);  // quick check next time
