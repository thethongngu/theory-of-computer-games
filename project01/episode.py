...
class episode:
    ...
    def take_turns(self, play, evil):
        ...
        if max(self.step_by_agent() + 1, 2) % 2 != 0:
            return play
        else:
            return evil
    ...
    def time(self, who=-1):
        if self.ep_moves:
            if who == action.slide.type:
                return sum([mv[2] for mv in self.ep_moves[slice(2, self.step_by_agent(), 2)]])  # action, reward, time usage
            if who == action.place.type:
                return self.ep_moves[0][2] + sum(
                    [mv[2] for mv in self.ep_moves[slice(1, self.step_by_agent(), 2)]])  # action, reward, time usage
...
if __name__ == '__main__':
    ...
    print("".join(
        [str(move[0]) + ("[" + str(move[1]) + "]" if move[1] else "") + ("(" + str(move[2]) + ")" if move[2] else "")
         for move in moves]))
